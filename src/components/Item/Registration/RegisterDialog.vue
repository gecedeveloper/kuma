...
export default {
  ...
  computed: {
    userIsRegistered () {
      return this.$store.getters.user.purchasedProducts.findIndex(itemId => {
  ...
  methods: {
    onAgree () {
      if (this.userIsRegistered) {
        this.$store.dispatch('unregisterUserFromMeetup', this.itemId)
      } else {
        this.$store.dispatch('registerUserForMeetup', this.itemId)
      }
    }
  }
}
